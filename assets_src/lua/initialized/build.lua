...
local OldBuild = require "verbs/build"
...
function Build:preExecute(unit, targetPos, strParam, endPos)
    ...
    local recruitableUnits = Build:getRecruitableTargets(unit);
...
function Build:execute(unit, targetPos, strParam, path)
    ...
    if (uc.id == "gold_camp") then
        ...
        table.insert(newUnit.loadedUnits, unit.id)
        unit.inTransport = true
        unit.transportedBy = newUnit.id
    end
